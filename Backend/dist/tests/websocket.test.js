...
(0, vitest_1.describe)('route simulation websocket', () => {
    ...
    (0, vitest_1.it)('streams simulation frames for a route', async () => {
        const flightService = new flightService_1.FlightService(new mockProviders_1.MockAirportRepository(), (0, mockProviders_1.createMockFlightPlanProvider)(), (0, mockProviders_1.createMockWeatherProvider)(), (0, mockProviders_1.createMockFuelProvider)(), (0, mockProviders_1.createMockFlightTrackingProvider)());
    ...
    (0, vitest_1.it)('streams live flight frames for selected aircraft', async () => {
        const flightService = new flightService_1.FlightService(new mockProviders_1.MockAirportRepository(), (0, mockProviders_1.createMockFlightPlanProvider)(), (0, mockProviders_1.createMockWeatherProvider)(), (0, mockProviders_1.createMockFuelProvider)(), (0, mockProviders_1.createMockFlightTrackingProvider)());
        const server = (0, node_http_1.createServer)((0, app_1.createApp)());
        servers.push(server);
        const socketServer = (0, liveFlightStream_1.attachLiveFlightWebSocket)(server, flightService);
        servers.push(socketServer);
        await new Promise((resolve) => server.listen(0, '127.0.0.1', resolve));
        const address = server.address();
        if (!address || typeof address === 'string')
            throw new Error('Expected server address');
        const frame = await new Promise((resolve, reject) => {
            const client = new ws_1.default(`ws://127.0.0.1:${address.port}/ws/flights/mock/mock-VIDP-0/live?routeId=generated-VIDP-VOBL&intervalMs=5000`);
            client.on('message', (data) => {
                resolve(JSON.parse(String(data)));
                client.close();
            });
        ...
        (0, vitest_1.expect)(frame.type).toBe('live-flight');
        (0, vitest_1.expect)(frame.flight).toEqual(vitest_1.expect.objectContaining({ provider: 'mock', latitude: vitest_1.expect.any(Number) }));
    });
});
